...
void visualize(int x = -1, int y = -1, int z = -1)
{
    ...
    for (int i = 0; i <= SCREEN_WIDTH - rectSize; i += rectSize)
    {
        ...
        if (complete)
        {
            SDL_SetRenderDrawColor(renderer, 255, 160, 110, 0);
        ...
        {
            SDL_SetRenderDrawColor(renderer, 255, 115, 200, 0);
            SDL_RenderFillRect(renderer, &rect);
        }
        else
        {
            SDL_SetRenderDrawColor(renderer, 180, 190, 199, 0);
            SDL_RenderDrawRect(renderer, &rect);
        }
        j++;
    }
...
void quickSort(int a[], int low, int high)
{
    if (low < high)
    {
        int partition_ind = partition_array(a, low, high);
        quickSort(a, low, partition_ind - 1);
        quickSort(a, partition_ind + 1, high);
    }
}


void merge2SortedArrays(int a[], int low, int high)
{
    int size_output = (low - high) + 1;
